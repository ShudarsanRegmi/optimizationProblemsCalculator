    ...
    while True:
        ...
        for j in range(len(list_of_vertices)):
            ...
            if f_x == max_f_x:
                ...
                list_of_vertices.pop(j)  # Remove vertex at index j
                # if this maximum yielding vertex is the derived vertex than terminate

                if list_of_vertices[j] in derived_vertices:
                    print("Terminating...")
                    # Find the average o
        ...
        if len(list_of_vertices) == 2:
            ...
            # Check if the new vertex was previously derived from other vertices


            # if new_vertex in all_vertices:
            # if new_vertex in all_vertices:
            #     print(f"Convergence reached. New vertex value is a previously derived vertex value.")
            #     mean_x = (list_of_vertices[0][0] + list_of_vertices[1][0]) / 2
            #     mean_y = (list_of_vertices[0][1] + list_of_vertices[1][1]) / 2
            #     final_value = float(f(mean_x, mean_y))  # Ensure final_value is a float
            #     print(f"Termination Point: ({mean_x:.2f}, {mean_y:.2f}) with functional value {final_value:.2f}")
